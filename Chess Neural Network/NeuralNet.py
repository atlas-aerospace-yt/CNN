...
class NeuralNetwork():
    ...
    def find_similarity(self, output, legal):
        ...
        for i in range(0, len(legal)):
            ...
            print(f"The amout of characters: {len(value)}, {value}, {legal[i]}")
            if len(value) == 4:
    ...
    def forwardChess(self, board, engine, legal):
        ...
        print(f"Closest move to: {y}, {legal[self.find_similarity(y, legal)]}")

        return y


    # a self learning functions for just chess
    def backwardChess(self, actual, board, engine, plot=False):
