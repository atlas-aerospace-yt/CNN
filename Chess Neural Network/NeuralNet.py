...
class NeuralNetwork():
    ...
    def backwardChess(self, engine, plot=False, iter=100):
        ...
        for i in range(0, iter):
            ...
            for item in os.listdir(f"Engine{engine}/Training"):
                ...
                biasGradient = 2 * c * self.leakyReluPrime(zThree)
                weightGradient = 2 * c * self.leakyReluPrime(zThree) @ aTwo.T

                biasLayerThree = biasLayerThree - (learnRate * biasGradient)
                weightsLayerThree = weightsLayerThree - (learnRate * weightGradient)

                biasGradient = (2 * c * self.leakyReluPrime(zThree)).T @ weightsLayerThree * self.leakyReluPrime(zTwo)
                weightGradient = (2 * c * self.leakyReluPrime(zThree)).T @ weightsLayerThree * self.leakyReluPrime(zTwo) @ aOne

                biasLayeTwo = biasLayerTwo - (learnRate * biasGradient)
                weightsLayerTwo = weightsLayerTwo - (learnRate * weightGradient)

                biasGradient = (2 * c * self.leakyReluPrime(zThree)).T @ weightsLayerThree * self.leakyReluPrime(zTwo) @ weightsLayerTwo * self.sigmoidPrime(zOne)
                weightGradient = (2 * c * self.leakyReluPrime(zThree)).T @ weightsLayerThree * self.leakyReluPrime(zTwo) @ weightsLayerTwo * self.sigmoidPrime(zOne) @ input
